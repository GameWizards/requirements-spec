...
Anttris is a game that offers both creative freedom and fierce competition. The game revolves around solving puzzle cubes which are composed of different types of blocks. Each block has a special property allowing for sophisticated puzzles to be created. Players can solve the puzzle cubes by interacting with these blocks in many ways, such as by removing blocks or setting off massive chain reactions.
...
To make our video game easy to use, the user interface must 
be intuitive to use. To make our game accessible to
the standard user, our standard input device will be a mouse. Touch screen 
input will also be supported in mobile devices.
...
Anttris is to be professional-grade software, it is necessary  that it run
quickly. As is the standard for video-games, Anttris
will deliver a minimum of 30 frames per second, and a max of 60 frames per
second. Any speed below this range on a modern-day computer is
unacceptable.

%How do we define a modern day computer? regular old office computer?
Our game Anttris is designed to be available to the common
...
Boundary objects are those that the user will be using directly through the user interface. These objects are responsible for displaying the correct buttons for the user and directing the player to the type of game play they selected. They also manage input actions such as dragging and clicking.

\begin{enumerate}
\item {\bf Host Match Button.} This object is used by the user through 
the user interface in order to host a multiplayer game. The hosting 
player is able to input the necessary fields in order to 	allow another 
player to connect to their game. The object shares the match ID with 
another player if they choose to accept the match and is 
responsible for sending the new process to Select Puzzle to ensure 
that both players are starting on new unsolved puzzles.
\item {\bf Join Match Button.} This object is used through the user 
interface in order to connect to another online player who is hosting 
a match in the multiplayer game. It is responsible for connecting the 
two players together and sending the new process to Select Puzzle to 
ensure that both players are starting  on new unsolved puzzles.
\item {\bf Select Puzzle.} This object is used by the user through the 
user interface in order to select which puzzle the player would like 
to solve the puzzle, edit the puzzle, or send the puzzle to another 
player when playing in the multiplayer mode. 
\item {\bf New Game Button.} This object is used by the user through 
the user interface in order to start a new puzzle in the offline mode. 
The object will send the new process to Select Puzzle to ensure that 
the user is starting on an unsolved puzzle.
\item {\bf Quit Game.} This object is used by the user through the 
user interface in order to close out of the game completely. It is 
responsible for sending the quit process to the main node when the 
button is pressed by the user.
\item {\bf Event Catcher.} This object is responsible for sending the 
quit process to the main node when the acceptable keys are inputted to 
the keyboard.
\end{enumerate}

\subsubsection{Entity Objects}

The entity objects are those that the user will be able to interact 
with but are responsible for storing or changing the data about the 
puzzle.

\begin{enumerate}
\item {\bf Block.} This object is responsible for creating new blocks 
and storing the properties about a selected block in the editor and 
running the action of the selected block in the solving process. 
\item {\bf PuzzleScene.} This object is responsible for any changes 
that are made to the configuration of the puzzle in the editor or 
solving and displaying to the user the changes that are made.
\item {\bf Camera.} This object is responsible for changing the view 
in which the puzzle is being viewed. It is responsible for adjusting 
the view of the puzzle and displaying which blocks can be viewed 
depending on the location of the gird.
\end{enumerate}

\subsubsection{Control Objects}

The control objects are those which coordinate the flow of game play by changing what is being displayed to the user based on the processes passed by the entity and boundary objects. 

\begin{enumerate}
\item {\bf BlockModifier.} This object is responsible for the creation 
of new blocks, deletion of blocks, and the modification of blocks 
during the editor process. It is responsible for sending the changes 
made to the block to the grid manager. 
\item {\bf GridManager.} This object is responsible for the changes 
made to the puzzle during the editor and puzzle solving processes. It 
is responsible for changing the puzzle being displayed based on the 
action passed to it, and sending the change in properties of the block 
space after the action has occurred.
\item {\bf BlockAction.} This object is responsible for executing the 
action of the block when the user selects it. It is responsible for 
sending the action based on the properties of the block to the grid 
manager during the puzzle solving process.
\end{enumerate}
...
% should've used \forloop
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_host_match.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_join_match.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_offline_play.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_edit.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_solve.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_navigate_puzzle.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5.5in]{sequence_quit.png}
